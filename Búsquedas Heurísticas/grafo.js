class Graph {
  ...
  removeNode(nodo)
  {
    // se copia todo el array del nodo que se va a borrar para saber con quienes estÃ¡ conectado.
  ...
  bfs(origen,destino){
    ...
    var queue=[]
    queue.push(origen); // se aÃ±ade el primer nodo a la cola
    while(queue.length){ // mientras que la cola no este vacia
        ...
        for(let i = 0; i<this.grafo[NodoActual[0]].length;i++){
            if(visitados[this.grafo[NodoActual[0]][i][0]]==0){
                ...
                queue.push(this.grafo[NodoActual[0]][i]);// y lo aÃ±ado al final de la cola
            }
        }
      }
      mostrarCamino(recorrido); 
  ...
  a_star_recursion(nodoActual, destino,visitados, recorrido){//Pasa todo el node
    ...
    for( let i=0;i<this.grafo[nodoActual[0]].length;i++){
        if(!visitados[this.grafo[nodoActual[0]][i][0]]){//Si no esta visitado lo agrega a las opciones
          let nodoSiguiente = [this.grafo[nodoActual[0]][i][0],this.getCoordenada(this.grafo[nodoActual[0]][i][0])]
          opciones.push({cost:(this.grafo[nodoActual[0]][i][1]+this.getDistancia(nodoSiguiente[1],destino[1])),destino:nodoSiguiente});
        }
    }
...
el_mejor_el_primero_recursion(nodoActual, destino,visitados,recorrido){
  addChange(nodoActual,"actual")
    if(nodoActual[0]==destino[0]){//Si encuentro el objetivo detiene la busqueda
        return true;
    }
    ...
    for(let i=0;i<opciones.length;i++){
        ...
        visitados[actual.destino[0]]=1
        recorrido.push(actual.destino);
        if(this.el_mejor_el_primero_recursion(actual.destino,destino,visitados,recorrido)) return true;
        //Si llega a un punto donde no puede avanzar retorna en la recursion, es poco probable
        recorrido.pop();
        //visitados[actual.destino[0]]=0
    }
  ...
  el_mejor_el_primero(origen, destino){
    //Paso 1, comprobar que el nodo exista
    if(this.grafo[origen[0]]==null || this.grafo[destino[0]]==null) return false;
    ...
    visitados[origen[0]]=1
    let recorrido = [origen]//Agrega el origen al inicio del recorrido
    let encontro = this.el_mejor_el_primero_recursion(origen, destino,visitados,recorrido)
    mostrarCamino(recorrido)
    return encontro;
  }
